...
//  Candyfloss
//
//  Created by Shihab Mehboob on 11/04/2025.
//

import UIKit
import ATProtoKit

class LikesViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UISearchResultsUpdating, UISearchBarDelegate {
        ...
        Task {
            do {
                if let atProto = GlobalStruct.atProto {
                    let x = try await atProto.getActorLikes(by: GlobalStruct.currentUser?.actorDID ?? "")
                    allPosts = x.feed
    ...
    func tableView(_ tableView: UITableView, contextMenuConfigurationForRowAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
        return UIContextMenuConfiguration(identifier: nil, previewProvider: nil) { _ in
            return makePostContextMenu(indexPath.row, post: self.allPosts[indexPath.row].post)
        }
    }
