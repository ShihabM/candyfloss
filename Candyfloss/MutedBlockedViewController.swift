//
//  MutedBlockedViewController.swift
//  Candyfloss
//
...
class MutedBlockedViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UISearchResultsUpdating, UISearchBarDelegate {
    
    var tableView = UITableView()
    var users: [AppBskyLexicon.Actor.ProfileViewDefinition] = []
    var type: UsersType = .muted
    enum UsersType {
        case muted
        case blocked
    }
        ...
        Task {
            do {
                if let atProto = GlobalStruct.atProto {
                    if type == .muted {
                        let x = try await atProto.getMutes()
                        users += x.mutes
                        currentCursor = x.cursor
                    } else {
                        let x = try await atProto.getGraphBlocks()
                        users += x.blocks
                        currentCursor = x.cursor
                    }
                ...
                print("Error fetching muted/blocked users: \(error)")
                DispatchQueue.main.async {
                    self.loadingIndicator.stopAnimating()
                    self.isFetching = false
                }
    ...
    func setUpTable() {
        ...
        self.searchController = ({
            ...
            if type == .muted {
                controller.searchBar.placeholder = "Search Muted"
            } else {
                controller.searchBar.placeholder = "Search Blocked"
            }
