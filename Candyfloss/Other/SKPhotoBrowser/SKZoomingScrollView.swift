...
//        indicatorView.frame = bounds
    ...
    open func setMaxMinZoomScalesForCurrentBounds() {
        ...
        if SKPhotoBrowserOptions.longPhotoWidthMatchScreen && imageView.frame.height >= imageView.frame.width {
            ...
            if let orientation = (UIApplication.shared.connectedScenes.first as? UIWindowScene)?.windows.first(where: { $0.isKeyWindow })?.windowScene?.interfaceOrientation, orientation.isPortrait {
